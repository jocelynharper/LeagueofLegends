...
.controller('ChampionCtrl', function($scope, $http, $state, $filter){
...
.controller('InfoCtrl', function($scope, $state, $http){
  $scope.champs = []
  $http.get('https://global.api.pvp.net/api/lol/static-data/na/v1.2/champion?champData=image,info,lore,skins&api_key=3cd4a072-2b3f-4169-b308-6af3a44066b9')
    .success(function(response){
      angular.forEach(response.data, function(champ){
        if(champ.name == $state.params.name){
          $scope.champs.push(champ);
